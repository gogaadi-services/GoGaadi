...
import { BadRequestException, HttpException, UnauthorizedException } from '@gogaadi/middleware';
import { prisma } from '@gogaadi/database';
import { PrismaClient } from '@prisma/client';
import { sendEmail, generateOtp } from '@gogaadi/config';

const JWT_SECRET = process.env.JWT_SECRET || 'gogaadi-jwt-secret-key';
  ...
  private handleAdminAction = async (req: Request, res: Response, action: string) => {
    ...
    switch (action) {
      ...
      case 'approve-role-request': {
        ...
        sendEmail(
          user.email,
          'gogaadi — Your Access Has Been Approved',
          `<h2>Welcome to gogaadi!</h2>
      ...
      case 'create-user': {
        ...
        sendEmail(
          email,
          'gogaadi — Your Account Has Been Created',
          `<h2>Welcome to gogaadi!</h2>
      ...
      case 'generate-temp-password': {
        ...
        for (const uid of userIds) {
          ...
          sendEmail(
            user.email,
            'gogaadi — Your Temporary Password',
