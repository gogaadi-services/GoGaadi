 ...
 * @returns {Object} Partner configuration object
 */
function loadPartnerEnv(root) {
  ...
  if (partner) {
    ...
    if (fs.existsSync(partnerFile)) {
      const config = JSON.parse(fs.readFileSync(partnerFile, 'utf-8'));
  ...
  if (fs.existsSync(defaultFile)) {
    const config = JSON.parse(fs.readFileSync(defaultFile, 'utf-8'));
    console.log(`🔵 Active PARTNER: ${config.partner} (ID: ${config.partnerId})`);
    return config;
  }
