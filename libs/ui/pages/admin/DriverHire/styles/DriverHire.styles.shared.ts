...
export const getBaseStyles = (theme: Theme): Record<string, CSSObject> => ({
  container: {
    ...
    background: 'linear-gradient(160deg, #f5f3ff 0%, #fafbff 50%, #ede9fe 100%)',
    minHeight: '100vh',
  ...
  pageHeader: {
    ...
    background: 'linear-gradient(135deg, #0f0a2e 0%, #2d1b69 30%, #6d28d9 65%, #8b5cf6 100%)',
    backgroundSize: '300% 300%',
    animation: 'dh-gradient-shift 8s ease infinite',
    borderRadius: 18,
    padding: theme.spacing(3.5, 4),
    position: 'relative',
    overflow: 'hidden',
    boxShadow:
      '0 24px 64px rgba(109,40,217,0.28), 0 8px 24px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.1)',
    '&::before': {
      content: '""',
      position: 'absolute',
      top: -100, right: -100,
      ...
      borderRadius: '50%',
      background: 'radial-gradient(circle, rgba(196,181,253,0.38) 0%, rgba(139,92,246,0.12) 50%, transparent 70%)',
      animation: 'dh-orb-drift 14s ease-in-out infinite',
      pointerEvents: 'none',
    },
    '&::after': {
      content: '""',
      ...
      animation: 'dh-orb-drift 10s ease-in-out infinite reverse',
      pointerEvents: 'none',
    },
  ...
  headerOrb3: {
    ...
    animation: 'dh-orb-drift 9s ease-in-out 2s infinite',
    pointerEvents: 'none',
  },
  ...
  statCard0: {
    animation: 'dh-slide-up 0.5s 0s ease both',
    '&::before': { background: 'linear-gradient(90deg, #6d28d9, #8b5cf6)' },
    '&:hover': { boxShadow: '0 12px 32px rgba(109,40,217,0.16)' },
  },
  statCard1: {
    animation: 'dh-slide-up 0.5s 0.08s ease both',
    '&::before': { background: 'linear-gradient(90deg, #f59e0b, #fbbf24)' },
    '&:hover': { boxShadow: '0 12px 32px rgba(245,158,11,0.16)' },
  },
  statCard2: {
    animation: 'dh-slide-up 0.5s 0.16s ease both',
    '&::before': { background: 'linear-gradient(90deg, #10b981, #34d399)' },
    '&:hover': { boxShadow: '0 12px 32px rgba(16,185,129,0.16)' },
  },
  statCard3: {
    animation: 'dh-slide-up 0.5s 0.24s ease both',
  ...
  statValue: {
    ...
    animation: 'dh-counter 0.7s cubic-bezier(0.34,1.56,0.64,1) both',
  ...
  searchField: {
    ...
    '& .MuiOutlinedInput-root': {
      ...
      '&:hover fieldset': { borderColor: 'rgba(109,40,217,0.35)' },
  ...
  selectionIndicator: {
    ...
    animation: 'dh-slide-up 0.55s cubic-bezier(0.34,1.56,0.64,1) both',
  },
});
