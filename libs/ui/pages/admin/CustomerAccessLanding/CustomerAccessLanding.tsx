...
const AUTOMOTIVE: CustomerCategory[] = ['petrol-bunk', 'ev-charging', 'showroom'];
const FINANCE: CustomerCategory[] = ['vehicle-finance', 'finance-broker', 'insurance'];
const PLATFORM: CustomerCategory[] = ['user'];
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, mt: 0.5 }}>
        ...
        >
          ...
          {/* Live dot on icon when has notifications */}
          {hasNotif && (
            <Box
              sx={{
                position: 'absolute',
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography sx={{ fontWeight: 800, fontSize: '1.05rem', lineHeight: 1.2, pr: hasNotif ? 5 : 0 }}>
            {item.displayName}
          </Typography>
      ...
      >
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.82rem' }}>
            Select a service type to review and approve onboarding requests
          </Typography>
        </Box>
        {/* Hero total badge */}
        {totalPending > 0 && (
...
export default CustomerAccessLanding;
